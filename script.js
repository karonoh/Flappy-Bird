...
var pipes = [];
const questions = ["What is the capital of France?", "What is the largest planet in our solar system?", "What is the smallest country in the world?"
];
const answers = ["Paris", "Jupiter", "Vatican City"];
var index = 0;
...
function make_bird_slow_and_fall() {
  ...
  if (bird.y > myCanvas.height - bird.MyImg.height) {
    bird.velocity_y = 0;
    if (game_mode == "running") game_mode = "question";
    else game_mode = "over";
  }
...
function display_game_over() {
  ...
  // Assuming questions and answers are already defined elsewhere in your code
  ...
  const nextQuestionAndAnswer = getNextQuestionAndAnswer();
  {  
      const currentQuestion =
        nextQuestionAndAnswer.question;
      const currentAnswer = nextQuestionAndAnswer.answer;
      index = (index + 1) % questions.length;
    return { question: currentQuestion, answer: currentAnswer }
    // Increment index and reset to 0 if it exceeds the length of the questions array
  }

  // You can now use getNextQuestionAndAnswer() function wherever you need to get the next question and its answer
...
function Do_a_Frame() {
  ...
  switch (game_mode) {
    ...
    case "question": {
      const answer = window.prompt(questions[index]);
      if (answer === answers[index]) {
        game_mode = "running";
        // TODO: instead of resetting, put the bird in a safe spot so the player can continue playing
        bird.y = myCanvas.height / 2;
         reset_game();
      } else {
        game_mode = "over";
    ...
    case "over": {
      make_bird_slow_and_fall();
      display_game_over();
      getNextQuestionAndAnswer(); //Add this line to call the function
      break;
    }
  }
}
